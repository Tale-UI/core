...
const getLinearRgbChannel = (value: number): number => {
  const channel = value / 255;
  return channel <= 0.04045 ? channel / 12.92 : Math.pow((channel + 0.055) / 1.055, 2.4);
};

const getLinearRgbUnitChannel = (channel: number): number =>
  channel <= 0.04045 ? channel / 12.92 : Math.pow((channel + 0.055) / 1.055, 2.4);

const getHexRelativeLuminance = (hex: string): number => {
  const h = hex.replace('#', '');
...
const getOklchRelativeLuminance = (l: number, c: number, h: number | undefined): number => {
...
export const generatePalette = (
  ...
  const shades = isNeutral ? NEUTRAL_SHADES : NAMED_SHADES;

  const L_MAX = 0.977;

  const DARK_L_RATIO = isNeutral ? 0.45 : 0.55;
  const DARK_C_RATIO = isNeutral ? 0.5 : 0.62;
  ...
  const neutralLightAnchorLuminance = isNeutral
    ? getOklchRelativeLuminance(L_MAX, neutralLightAnchorChroma, H60)
    : null;
  const neutralBaseLuminance = isNeutral ? getOklchRelativeLuminance(L60, C60, H60) : null;
  ...
  return shades.map((shade) => {
    ...
    if (shade < 60) {
      if (
        isNeutral &&
        shade >= 10 &&
        neutralLightAnchorLuminance !== null &&
        neutralBaseLuminance !== null
      ) {
...
export const randomBaseColor = (mode: 'named' | 'neutral' = 'named'): string => {
  ...
  for (let attempt = 0; attempt < 200; attempt += 1) {
    ...
    const c = isNeutral ? 0.01 + Math.random() * 0.03 : 0.1 + Math.random() * 0.15;

    if (isNeutral) {
      const l = 0.35 + Math.random() * 0.2;
