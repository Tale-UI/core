...
export const randomBaseColor = (mode, { whiteAnchor = false } = {}) => {
  ...
  for (let attempt = 0; attempt < 200; attempt += 1) {
    ...
    const c = isNeutral
      ...
      : 0.1 + Math.random() * 0.15; // 0.10–0.25

    if (isNeutral) {
      const l = 0.35 + Math.random() * 0.2; // 0.35–0.55
 ...
 */
export const getRadiusValue = (multiplier, curvature) => multiplier * RADIUS_SCALE * curvature;
...
export const generateCssOutput = (name, palette, { mode = 'named' } = {}) => {
  ...
  if (mode === 'named') {
    ...
    if (s5Hex && s100Hex) {
      ...
      const darkOverrides = [];
      for (let i = 0; i < NAMED_SHADES.length; i += 1) {
