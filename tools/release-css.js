...
function main() {
  ...
  if (tagExistsLocally(tag) || tagExistsRemotely(tag)) {
    fail(
      `Tag ${tag} already exists. Choose a different version with --version (for example --version 1.0.2).`,
    );
  }
  ...
  try {
    ...
    run(
      `gh release create ${tag} --title "${tag} - Publish to NPM" --notes "Release ${tag} of ${packageName}." --repo Tale-UI/tale-ui`,
    );
