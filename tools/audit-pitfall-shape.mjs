...
import {
  componentTokensForSlug,
  normalizeForComparison,
  slugSummaryMatch,
  stripBacktickContent,
} from './pitfall-shape-rules-lib.mjs';
...
const GLOBAL_ONLY_SLUG_PREFIXES = ['always-generate', 'never-output', 'always-output'];

const DISALLOWED_SUB_BULLETS = ['common mistake', 'enforcement rule'];
...
function parseBlocks(content, filePath) {
  ...
  while (i < lines.length) {
    ...
    const id = idMatch[1];
    const proseOnly =
      commentBlock.includes('<!-- prose-only -->') || lines[j]?.includes('<!-- prose-only -->');
    const multiIdeaOk =
      commentBlock.includes('<!-- multi-idea-ok -->') ||
      lines[j - 1]?.includes('<!-- multi-idea-ok -->');
    ...
    while (k < lines.length) {
      const l = lines[k];
      if (l.startsWith('<!-- pitfall:') || l.startsWith('## ')) {
        break;
      }
      rawBlockLines.push(l);
      if (/^- \*\*/.test(l)) {
        bulletLines.push({ line: l, lineNum: k + 1 });
      }
      k += 1;
    }
...
function checkBlock(block, isPitfallsDoc, violations) {
  ...
  const componentTokens = isPitfallsDoc ? [] : componentTokensForSlug(fileBasename);
  ...
  if (bulletLines.length > 1) {
    violations.push({
      loc,
      id,
      msg: `${bulletLines.length} top-level bullets found (expected exactly 1)`,
    });
  }
  ...
  if (!bulletMatch) {
    violations.push({
      loc,
      id,
      msg: 'first bullet does not match "- **summary** — detail" format',
    });
    return;
  }
  ...
  if (!multiIdeaOk) {
    ...
    if (dashCount >= 2) {
      violations.push({
        loc,
        id,
        msg: `summary appears multi-idea (${dashCount} em-dashes outside backticks)`,
      });
    }
    if (/\band\b/.test(stripped)) {
      violations.push({
        loc,
        id,
        msg: 'summary may be multi-idea (contains "and" outside backticks)',
      });
    }
    if (/;/.test(stripped)) {
      violations.push({
        loc,
        id,
        msg: 'summary may be multi-idea (contains semicolon outside backticks)',
      });
    }
  }
  ...
  if (!slugSummaryMatch(id, summary, { componentTokens })) {
    violations.push({
      loc,
      id,
      msg: `slug tokens not reflected in summary (slug: "${id}", summary: "${summaryClean.slice(0, 80)}...")`,
    });
  }
  ...
  if (!proseOnly) {
    ...
    if (!hasAntiPattern) {
      violations.push({
        loc,
        id,
        msg: 'missing anti-pattern sub-bullet (add <!-- prose-only --> if intentional)',
      });
    }
    if (!hasFix) {
      violations.push({
        loc,
        id,
        msg: 'missing fix sub-bullet (add <!-- prose-only --> if intentional)',
      });
    }
  }

  // Check 7: backtick discipline in anti-pattern/fix
  const apLines = [...rawBlock.matchAll(/^\s+- anti-pattern:\s*(.+)$/gm)].map((m) => m[1]);
  const fixLines = [...rawBlock.matchAll(/^\s+- fix:\s*(.+)$/gm)].map((m) => m[1]);
  for (const snippet of [...apLines, ...fixLines]) {
    ...
    if (!isBtWrapped && !isFenceStart && snippet.trim() !== '') {
      violations.push({
        loc,
        id,
        msg: `anti-pattern/fix snippet not backtick-wrapped: ${snippet.slice(0, 60)}`,
      });
    }
  }

  // Check 8: complete example shape
  const ceBlockMatch = rawBlock.match(
  ...
  if (ceBlockMatch) {
    ...
    if (!ceContent.includes('```')) {
      ...
      if (!ceContent.includes('return') && !ceContent.includes('export function')) {
        violations.push({
          loc,
          id,
          msg: 'complete example should contain a return statement or export function',
        });
      }
    }
    ...
    violations.push({
      loc,
      id,
      msg: 'complete example uses legacy inline backtick form — convert to fenced block',
    });
  }
  ...
  if (isPitfallsDoc && !hasCategory) {
    violations.push({
      loc,
      id,
      msg: 'missing <!-- category: ... --> comment (required in pitfalls.md)',
    });
  }
  if (!isPitfallsDoc && hasCategory) {
    violations.push({
      loc,
      id,
      msg: '<!-- category: ... --> comment should not appear in component docs',
    });
  }
    ...
    for (const prefix of GLOBAL_ONLY_SLUG_PREFIXES) {
      if (id.startsWith(prefix)) {
        violations.push({
          loc,
          id,
          msg: `slug "${id}" is a meta-pitfall that should only live in docs/pitfalls.md`,
        });
      }
    }
  }
...
function autoFixFile(filePath) {
  ...
  const after = before.replace(/^(\s+- (?:anti-pattern|fix):\s*.*?)``(?!`)(\s*)$/gm, '$1`$2');
  if (after !== before) {
...
if (allViolations.length === 0) {
  process.stdout.write(
    `✅ audit-pitfall-shape: no violations found (${componentFiles.length + 1} files checked)\n`,
  );
  process.exit(0);
} else {
