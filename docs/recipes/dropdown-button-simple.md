...
export function ButtonDropdown() {
  return (
    <Menu.Root onAction={(key) => { console.log('selected', key); }}>
...
- `Menu.Root onAction` receives the `id` of the selected item as a string. Use a `switch` statement or object map to dispatch the right handler.
