...
5. **Common pitfalls to avoid:**
   - `Table`, `Drawer`, `Meter`, `ProgressBar`, `NumberField`, etc. are namespace objects — always use `<Component.Root>`, never `<Component>` directly. Simple (non-namespace) components include: `Button`, `Link`, `Icon`, `IconButton`, `ColorModeToggle`, `Separator`, `ToggleButton`, `ToggleButtonGroup`, `RadioGroup`, `Form`, `ColorSwatch`, `FileTrigger`, `DropZone`, `Container`.
