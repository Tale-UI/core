...
5. **Common pitfalls to avoid:**
   ...
   - **`Text` component import** — always import `Text` explicitly: `import { Text } from '@tale-ui/react/text'`. `Text` is also a global Web API type (DOM); if the import is missing TypeScript resolves to that global, producing "'Text' cannot be used as a JSX component" errors.
   - **`Text` component `color` prop** only accepts `'default'` (default), `'muted'` (renders in `--neutral-60`), or `'accent'` (renders in `--color-60`). There is no `'secondary'`, `'tertiary'`, or other color value. To render muted/secondary text use `color="muted"`.
