...
for (const { id, label } of STORIES) {
  test(`${label} Default story`, async ({ page }) => {
    // In Storybook 10, /iframe.html redirects to /iframe (query params preserved).
    // Navigate directly to /iframe to avoid the redirect dropping the story ID.
    await page.goto(`/iframe?id=${id}&viewMode=story`, { waitUntil: 'networkidle' });
    ...
    await expect(page.locator('#storybook-root')).not.toContainText('No Preview', { timeout: 10_000 });
